...
def index():
    ...
    if request.method == 'POST':
        ...
        for key in request.form.keys():
            ...
            clean_form_data[key] = values if len(values) > 1 else values[0]
        ...
        if additional_pu_strip == "Yes":
            try:
                additional_distances = [
                    int(x) for x in request.form.getlist('additional_distances')
                    if x.strip().isdigit()
                ]
            except Exception as e:
                return f"Error parsing additional_distances: {str(e)}"
        ...
        # Generate PDF from image and save in memory
        pdf_io = BytesIO()
        ...
        x = (A4[0] - new_width) / 2
        y = (A4[1] - new_height) / 2
        c.drawImage(ImageReader(BytesIO(img_bytes)), x, y, width=new_width, height=new_height)
        c.showPage()
        c.save()
        pdf_io.seek(0)

        key = f"{harp_name}_{datetime.now().timestamp()}"
        
        if action == 'download':
            return send_file(
        BytesIO(pdf_io.getvalue()),
        mimetype='application/pdf',
        as_attachment=True,
        download_name=f"{harp_name}.pdf"
    )


            
        if action == 'edit':
            return render_template('index.html', form_data=clean_form_data)
...
def generate_pdf():
    ...
    for key in form.keys():
        print(f"{key}: {form.getlist(key)}")
    print("==========================")

    try:
        width = int(form['width'])
        height = int(form['height'])
        pu_quantity = int(form['pu_quantity'])
        harp_name = form['harp_name'].strip()
        version_number = form['version_number'].strip()
        additional_message = form.get('additional_message', '').strip()
        background_type = form['background_type']
        overlap = form.get('overlap', 'No')
        center_overlap = form.get('center_overlap', 'No')
        poly_ridge = form.get('poly_ridge', '')
        center_holes = form.get('center_holes', 'No')
        num_center_holes = int(form.get('num_center_holes', '0'))

        if center_overlap == "Yes":
            left_raw = form.get('left_spacing')
            right_raw = form.get('right_spacing')

            if left_raw and left_raw.startswith("["):
                left_values = [int(x) for x in ast.literal_eval(left_raw)]
                right_values = [int(x) for x in ast.literal_eval(right_raw)]
            else:
                left_values = [int(x) for x in form.getlist('left_spacing')]
                right_values = [int(x) for x in form.getlist('right_spacing')]

            spacing_values = left_values + right_values
        else:
            spacing_raw = form.get('spacing')
            if spacing_raw and spacing_raw.startswith("["):
                spacing_values = [int(x) for x in ast.literal_eval(spacing_raw)]
            else:
                spacing_values = [int(x) for x in form.getlist('spacing')]

        hole_distances = [int(x.strip()) for x in form.get('hole_distances', '').split(',') if x.strip()] if center_holes == "Yes" else []

        additional_pu_strip = form.get('additional_pu_strip', 'No')
        distances_raw = form.get('additional_distances')
        if distances_raw and distances_raw.startswith("["):
            additional_distances = [int(x) for x in ast.literal_eval(distances_raw)]
        else:
            additional_distances = [int(x) for x in form.getlist('additional_distances') if x.strip().isdigit()]
        hook_number = form.get('hook_type')
        magnified_image = form.get('magnified_image')
        pu_sickness_image = form.get('pu_sickness_image')
        center_strip_image = form.get('center_strip_image')
    ...
    image_data = generate_image(width, height, background_type, spacing_values,
        overlap, center_overlap, center_holes, num_center_holes, hole_distances,
        pu_quantity, additional_message, additional_pu_strip, additional_distances,
        poly_ridge, hook_number=hook_number,
        magnified_image=magnified_image,
        pu_sickness_image=pu_sickness_image,
        center_strip_image=center_strip_image,
        harp_name=harp_name,
        version_number=version_number)
    ...
    c = pdf_canvas.Canvas(pdf_io, pagesize=A4)
    scale = min(A4[0] / image.width, A4[1] / image.height) * 0.95
