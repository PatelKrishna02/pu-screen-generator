document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateDistanceFields() {
        const puQuantity = parseInt(document.getElementById('pu_quantity').value);
        const centerOverlap = document.getElementById('center_overlap').value;
        const container = document.getElementById('distance_fields_container');
    
        const leftInputs = document.querySelectorAll('input[name="left_spacing"]');
        const rightInputs = document.querySelectorAll('input[name="right_spacing"]');
        const regularInputs = document.querySelectorAll('input[name="spacing"]');
        ...
        if (centerOverlap === "Yes") {
            const leftCount = Math.floor(puQuantity / 2);
            const rightCount = puQuantity - leftCount;
            const leftFields = leftCount + 1;
            const rightFields = rightCount + 1;
    
            let html = '<div class="side-container" id="left_container"><h3>Left Side Distances</h3>';
            for (let i = 0; i < leftFields; i++) {
                const oldLeftVal = leftInputs[i]?.value || '';
                html += `<label>Left Distance ${i + 1}:</label>`;
                html += `<input type="text" name="left_spacing" value="${oldLeftVal}" required><br><br>`;
            }
            html += '</div>';
    
            html += '<div class="side-container" id="right_container"><h3>Right Side Distances</h3>';
            for (let j = 0; j < rightFields; j++) {
                const oldRightVal = rightInputs[j]?.value || '';
                html += `<label>Right Distance ${j + 1}:</label>`;
                html += `<input type="text" name="right_spacing" value="${oldRightVal}" required><br><br>`;
            }
            html += '</div>';
    
            container.innerHTML = html;
        } else {
            const totalFields = puQuantity + 1;
            let html = '<h3>Spacing Distances</h3>';
            for (let k = 0; k < totalFields; k++) {
                ...
                const oldVal = regularInputs[k]?.value || '';
                html += `<input type="text" name="spacing" value="${oldVal}" required><br><br>`;
            }
            container.innerHTML = html;
        }
    }
    ...
    function generateCheckboxes() {
        ...
        checkboxContainer.style.display = (additionalPUSelect.value === "Yes" && index > 0) ? "block" : "none";
